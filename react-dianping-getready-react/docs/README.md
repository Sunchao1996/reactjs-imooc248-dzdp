...
创建`./app/containers/Hello/userinfo.jsx`文件，将之前创建组件代码复制进去
...
注意，代码`import Hello from './containers/Hello';`这里可以写成`./containers/Hello/userinfo.jsx`也可以写成`./containers/Hello/index`

### subpage 层

如果`Hello`组件再稍微复杂一点，那么把代码都放一块也会变得复杂，接下来我们再拆分。

创建`./app/containers/Hello/subpage`目录，然后在其下创建三个文件`Carousel.jsx` `Recommend.jsx` `List.jsx`，分别写入相应的代码（看代码文件即可），然后`./app/containers/Hello/userinfo.jsx`中即可这样写
...
创建`./app/components/Header/userinfo.jsx`文件，简单写入一个组件的代码（见源码文件），然后在`./app/containers/userinfo.jsx`中引用
